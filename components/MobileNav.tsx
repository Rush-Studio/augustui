    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent side="left">
        ...
        >
          ...
          <span
            className={cn(josefinSans.className, "translate-y-[2px] font-bold")}
          >
            {siteConfig.name}
          </span>
        </Link>
        <ScrollArea className="my-4 h-[calc(100vh-120px)]">
          ...
          <div className="flex flex-col gap-y-2">
            {docsConfig.sidebarNav.map((item, index) => (
              <div key={index} className="flex flex-col gap-y-1.5 pt-6">
                <h4 className={cn("font-medium", josefinSans.className)}>
                  {item.title}
                </h4>
                <div className="ml-1 flex flex-col gap-y-1.5 border-l border-border pl-1 pr-4">
                  {item.items?.map((item) =>
    ...
    <SheetClose asChild>
      <Link
        ...
        className={cn(
          ...
          isActive ? "rounded-md bg-secondary font-medium text-primary" : "",
        )}
        {...props}
      >
        {children}
